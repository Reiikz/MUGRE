...
const byte MOTOR_ENABLE_PIN = 19;
const byte LEFT_MOTOR_INVERT_PIN = 18;
const byte RIGHT_MOTOR_INVERT_PIN = 5;
...
void DoBlinkRoutine()
{
  ...
  switch(actual_blink_routine)
  {
    case BLINK_ROUTINE_STOPPED:
    {
      st_brake_lights = HIGH;
      if(actual_time - lu_brake_lights > BLINK_INTERVAL)
      {
        lu_brake_lights = actual_time;
        st_left_turn_light = !st_left_turn_light;
        st_right_turn_light = !st_right_turn_light;
      }
      break;
    }

    case BLINK_ROUTINE_TURNING_LEFT:
    {
      if(actual_time - lu_left_turn_light > BLINK_INTERVAL)
      {
        lu_left_turn_light = actual_time;
        st_left_turn_light = ! st_left_turn_light;
        st_right_turn_light = LOW;
      }
      break;
    }

    case BLINK_ROUTINE_TURNING_RIGHT:
    {
      if(actual_time - lu_right_turn_light > BLINK_INTERVAL)
      {
        lu_right_turn_light = actual_time;
        st_left_turn_light = LOW;
        st_right_turn_light = !st_right_turn_light;
      }
      break;
    }

    case BLINK_ROUTINE_REVERSE:
    {
      st_brake_lights = HIGH;
      st_left_turn_light = LOW;
      st_right_turn_light = LOW;
    ...
    case BLINK_ROUTINE_WIFI_DISCONNECTED:
    {
      if(actual_time - lu_brake_lights > BLINK_INTERVAL)
      {
        lu_brake_lights = actual_time;
        st_brake_lights = !st_brake_lights;
        st_left_turn_light = !st_brake_lights;
        st_right_turn_light = !st_brake_lights;
      }
      break;
    }

    case BLINK_ROUTINE_NOTIFY:
    {
      for(byte b = 0; b < 10; b++)
      {
        st_brake_lights = !st_brake_lights;
...
void UpdateLights()
{
  digitalWrite(BRAKE_LIGHT_PIN, st_brake_lights);
  digitalWrite(LEFT_TURN_LIGHT_PIN, st_left_turn_light);
  digitalWrite(RIGHT_TURN_LIGHT_PIN, st_right_turn_light);
}
